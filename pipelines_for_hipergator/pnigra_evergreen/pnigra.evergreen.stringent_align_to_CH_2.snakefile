...
rule bwa_map:
	input:
		#ref=config["reference"]["Pop.trichocarpa"]["v3"],
		#ref=config["reference"]["Pop.nigra"]["old"],
		ref=config["reference"]["Pop.nigra"]["supernova_CH_2"],
		#ref=config["reference"]["Pop.nigra"]["supernova_PA"],	
		fq1="/ufrc/kirst/share/Pnigra_Sempervirens/raw/{sample}/{sample}_R2_reverse_paired.trim.fq.gz",
		fq2="/ufrc/kirst/share/Pnigra_Sempervirens/raw/{sample}/{sample}_R1_forward_paired.trim.fq.gz"
	output:
		aln_1="mapped_reads/{sample}.R1.sai",
		aln_2="mapped_reads/{sample}.R2.sai",
		bam=protected("mapped_reads/{sample}.bam")
	params:
	...
	threads: 10
	shell:
		"module load bwa; module load samtools; (bwa aln -t {threads} -n 1.0 {input.ref} {input.fq1} > {output.aln_1} ; bwa aln -t {threads} -n 1.0 {input.ref} {input.fq2} > {output.aln_2} ; bwa sampe -r '{params.rg}' {input.ref} {output.aln_1} {output.aln_2} {input.fq1} {input.fq2} | samtools view -Sb - > {output.bam}) 2> {log}"
...
rule samtools_sort_bams:
	...
	shell:
		"module load samtools; samtools sort {input} > {output} 2> {log}"
...
rule samtools_rmdup:
	...
	shell:
		"module load samtools; samtools rmdup {input} {output} 2> {log}"
...
rule freebayes:
	input:
		...
		#ref=config["reference"]["Pop.nigra"]["old"],
		#ref=config["reference"]["Pop.nigra"]["supernova_PA"],
		ref=config["reference"]["Pop.nigra"]["supernova_CH"],
