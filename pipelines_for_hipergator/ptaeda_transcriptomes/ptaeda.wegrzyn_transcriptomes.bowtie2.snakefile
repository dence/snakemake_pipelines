...
rule bowtie2_align:
	...
	params:
		...
		rg='--rg-id={sample} --rg "SM:{sample}" --rg "LIB:{sample}" --rg "PL:illumina"'
		#rg='-p 20 --rg-id={sample} --rg-sample={sample} --rg-library={sample} --rg-platform=illumina'
	benchmark:
		"benchmarks/{sample}.bowtie2.benchmark.txt"
	shell:
		"module load bowtie2; export BOWTIE2_INDEXES=""; bowtie2 -q -t --mm -p {threads} -x /home/d.ence/projects/ref_genomes/loblolly_pine/V1_1/greater_than_3k_ref/ptaeda.v1.01.fa.masked.longer_than3k.fasta {params.rg} -S {output} -1 {input.fq1} -2 {input.fq2}"
...
rule report:
	input:
		...
		bams=expand("realigned_bams/{sample}.sorted.rmdup.realigned.bam",sample=config["samples"])
	output:
		"freebayes_calls.report.html"
	run:
		from snakemake.utils import report
		with open(input.bams) as gtf:
