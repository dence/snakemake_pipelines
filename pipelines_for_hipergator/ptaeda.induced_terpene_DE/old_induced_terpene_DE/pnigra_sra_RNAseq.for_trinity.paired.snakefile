rule all:
	input:
		"trinity_guided_assembly/Trinity." + config["accession"][0]  + ".fasta"	
...
rule dump_accessions:
	input:
		...
		"sra/sra/{sample}.sra"	
	output:
		#"tmp_fastqs/{sample}_1.fastq.bz2"
		temp("tmp_fastqs/{sample}_1.fastq.bz2"),
		temp("tmp_fastqs/{sample}_2.fastq.bz2"),
		"logs/fastq_dump/{sample}.log"
	log:
		"logs/fastq_dump/{sample}.log"
	shell:
		"module load sra; fastq-dump --split-files --bzip2 --outdir tmp_fastqs {input} &> {log}"
rule pretrim_fastqc:
	input:
		#"tmp_fastqs/{sample}_1.fastq.bz2"
		"tmp_fastqs/{sample}_1.fastq.bz2",
		"tmp_fastqs/{sample}_2.fastq.bz2"
	output:
...
rule trim_reads:
	input:
		fq1="tmp_fastqs/{sample}_1.fastq.bz2",
		fq2="tmp_fastqs/{sample}_2.fastq.bz2",
	...
	shell:
		"module load gcc/5.2.0;  module load cutadapt; cutadapt -u -10 -U -10 -q30,30 --minimum-length=50 -o {output.fq1} -p {output.fq2} {input.fq1} {input.fq2} &> {log}"
		#"module load cutadapt; cutadapt -u -10 -q30,30 --minimum-length=50 -o {output.fq1} {input.fq1} &> {log}"
rule posttrim_fastqc:
	input:
		#"tmp_fastqs/{sample}_1.fastq.bz2"
		temp("trimmed_reads/{sample}/{sample}.trimmed.R1.fastq.gz"),
		temp("trimmed_reads/{sample}/{sample}.trimmed.R2.fastq.gz")
		#"tmp_fastqs/{sample}_1.fastq.bz2",
		#"tmp_fastqs/{sample}_2.fastq.bz2"
	output:
...
rule star_align:
	...
	threads:
		10
	shell:
		"module load samtools; module load gcc/5.2.0; module load star/2.6.0a; STAR --runThreadN {threads} --genomeDir {input.ref} --readFilesIn {input.fq1} {input.fq2} --readFilesCommand zcat -c --outFileNamePrefix {params.prefix} --outSAMtype BAM Unsorted > {log}"
		#"module load samtools; load star; STAR --runThreadN {threads} --genomeDir {input.ref} --readFilesIn {input.fq1} {input.fq2} --readFilesCommand bunzip2 -c --outFileNamePrefix {log} | samtools view -bS - > {output}"
...
#		fq2=expand("trimmed_reads/{sample}/{sample}.trimmed.R1.fastq.gz",sample=config["samples"])	
	params:
		"--seqType fq --SS_lib_type " + config["layout"][0] + " --max_memory 50G --genome_guided_max_intron 20000"
#		"--seqType fq --SS_lib_type F --max_memory 50G"
	output:
		output_dir="trinity_guided_assembly",
		output_fasta="trinity_guided_assembly/Trinity." + config["accession"][0] + ".fasta",	
	...
	threads: 
		10
	shell:
		"module load gcc/5.2.0; module load trinity/r20180213-2.6.5; Trinity --normalize_reads --CPU {threads} --genome_guided_bam {input.merged_bam} --output {output.output_dir} {params} &> {log}"
		#"gcc/5.2.0; module load trinity/r20180213-2.6.5; Trinity {params} --single {input.fq1} --CPU {threads} --genome_guided_bam {input.bam} --output {output.output_dir} &> {log}"
